...
public class FontSpec {
    ...
    private boolean absoluteSize = false;
    private int underlineStyle;


    public FontSpec() {
        this.size = CWidget.getBaseFontSize();
        this.color = CWidget.getPalette().REGULAR_TEXT;
    }
    ...
    public int getSize(){
        if (absoluteSize) {
            ...
            return CWidget.getBaseFontSize();
        }
    }
    ...
    public void setAbsoluteSize(boolean b) {
        this.absoluteSize = b;
    }


    public boolean getAbsoluteSize() {
        return this.absoluteSize;
    }


    public String getKey(){
        if ((this.isKeyInvalidated || CWidget.isFontCacheInvalidated()) && this.name != null){
		    this.key = String.format("%s_%d_%d_%d%d%d%d%d%d",
				this.name,
				this.color,
				this.getSize(),
				antialias?1:0,
    ...
    public FontSpec duplicate(){
        ...
        fontOptions.setAbsoluteSize(absoluteSize);
        return fontOptions;
    }
    ...
    public FontSpec mergeFrom(FontSpec from){
        ...
        this.absoluteSize = from.getAbsoluteSize();
        this.isKeyInvalidated = true;
        return this;
    }
}
